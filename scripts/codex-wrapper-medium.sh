#!/usr/bin/env bash
#
# codex-wrapper-medium.sh - Codex CLI ラッパー（軽量タスク用: medium effort）
#
# 目的:
#   軽量タスク用のCodex呼び出し。reasoning effort = medium で実行。
#   ドキュメント生成、簡易修正などコスト削減が望ましいタスク向け。
#
# 使用方法:
#   cat prompt.md | ./scripts/codex-wrapper-medium.sh --stdin > output.md
#
# 使い分け:
#   - 軽量タスク（ドキュメント生成、簡易修正）: codex-wrapper-medium.sh (このファイル)
#   - Coder（実装）: codex-wrapper-high.sh
#   - Reviewer（レビュー）: codex-wrapper-xhigh.sh
#
set -euo pipefail
...
readonly FIXED_REASONING_EFFORT="medium"  # 軽量タスク用: medium

# =====================================================
# ログ出力
# =====================================================
log_info() {
  echo "[codex-wrapper-medium] INFO: $*" >&2
}

log_warn() {
  echo "[codex-wrapper-medium] WARN: $*" >&2
}
...
main() {
  if ! command -v codex >/dev/null 2>&1; then
    echo "[codex-wrapper-medium] ERROR: codex CLI not found in PATH" >&2
    exit 1
  fi
  ...
  if [[ "$cmd_type" == "resume" ]]; then
    if [[ -z "$session_id" ]]; then
      echo "[codex-wrapper-medium] ERROR: --resume requires session_id" >&2
      exit 1
    fi
