#!/usr/bin/env bash
#
# codex-wrapper-high.sh - Codex CLI ラッパー（Coder用: high effort）
#
# 目的:
#   Coderタスク用のCodex呼び出し。reasoning effort = high で実行。
...
#
# 使い分け:
#   - Coder（実装）: codex-wrapper-high.sh (このファイル)
...
readonly FIXED_REASONING_EFFORT="high"  # Coder用: high

# =====================================================
# ログ出力
# =====================================================
log_info() {
  echo "[codex-wrapper-high] INFO: $*" >&2
}

log_warn() {
  echo "[codex-wrapper-high] WARN: $*" >&2
}
...
main() {
  if ! command -v codex >/dev/null 2>&1; then
    echo "[codex-wrapper-high] ERROR: codex CLI not found in PATH" >&2
    exit 1
  fi
  ...
  if [[ "$cmd_type" == "resume" ]]; then
    if [[ -z "$session_id" ]]; then
      echo "[codex-wrapper-high] ERROR: --resume requires session_id" >&2
      exit 1
    fi
